#!/usr/bin/env python3
"""
Upload TRIDENT Contours to Google Drive Script

This script collects contour visualization outputs from TRIDENT processing
...
def find_contour_files(trident_output_dir):
    """
    Find all contour visualization files generated by TRIDENT.
    """
    trident_path = Path(trident_output_dir)
    contour_files = []
    
    # Look for images in the contours directory
    contours_dir = trident_path / "contours"
    if contours_dir.exists():
        for ext in ['.jpg', '.jpeg', '.png']:
            contour_files.extend(contours_dir.glob(f"*{ext}"))
    
    # If no files found in contours directory, try alternate paths
    if not contour_files:
        # Try other possible directory names
        alternate_dirs = ["contour", "contours_viz", "contours_visualization"]
        for dir_name in alternate_dirs:
            alt_dir = trident_path / dir_name
            if alt_dir.exists():
                for ext in ['.jpg', '.jpeg', '.png']:
                    contour_files.extend(alt_dir.glob(f"*{ext}"))
    
    # Also look for contour files in any subdirectory
    if not contour_files:
        for item in trident_path.rglob("*"):
            if item.is_file() and item.suffix.lower() in ['.jpg', '.jpeg', '.png']:
                if 'contour' in str(item).lower():
                    contour_files.append(item)
...
def main():
    parser = argparse.ArgumentParser(description="Upload TRIDENT contour visualizations to Google Drive.")
    parser.add_argument("--trident_output_dir", type=str, required=True,
                      help="Directory containing TRIDENT output with contour visualization files.")
    ...
    parser.add_argument("--folder_name", type=str, default="PDAC5_QC_Contours",
    ...
    # Find contour visualization files
    logger.info("Searching for contour visualization files...")
    contour_files = find_contour_files(trident_output_dir)
    
    if not contour_files:
        logger.warning("No contour visualization files found in TRIDENT output directory")
        return

    logger.info(f"Found {len(contour_files)} contour files:")
    for viz_file in contour_files:
